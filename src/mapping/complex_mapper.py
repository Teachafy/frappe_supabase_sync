...
class ComplexMapper:
    ...
    async def handle_complex_mapping(self, value: Any, config: Dict[str, Any],
                                   direction: str, source_system: str = None, target_system: str = None) -> Any:
        ...
        try:
            ...
            if mapping_type == "array_to_uuid_array":
                ...
                return await self._handle_company_to_org_mapping(value, config, direction)
            elif mapping_type == "string_to_int":
                return await self._handle_string_to_int_mapping(value, config, direction)
            elif mapping_type == "reverse_name_combination":
                return await self._handle_reverse_name_combination(value, config, direction)
    ...
    async def _handle_email_priority(self, value: Any, config: Dict[str, Any], 
        ...
        try:
            if direction == "frappe_to_supabase":
                ...
                email_fields = config.get("email_priority", ["personal_email", "company_email", "preferred_contact_email"])
    ...
    async def _handle_company_to_org_mapping(self, value: Any, config: Dict[str, Any],
                                           direction: str) -> Any:
    ...
    async def _handle_array_mapping(self, value: Any, config: Dict[str, Any], 
                                   direction: str) -> Any:
    ...
    async def _handle_boolean_mapping(self, value: Any, config: Dict[str, Any], 
                                     direction: str) -> Any:
    ...
    async def _handle_datetime_mapping(self, value: Any, config: Dict[str, Any], 
                                      direction: str) -> Any:
    ...
    async def _handle_complex_mapping(self, field_name: str, value: Any, 
                                     mapping_config: Dict[str, Any], 
                                     direction: str) -> Any:
        """Handle complex mapping with error handling"""
        try:
            return await self.map_complex_field(field_name, value, mapping_config, direction)
        except Exception as e:
            logger.error(f"Complex mapping failed for {field_name}", error=str(e))
            return value
    
    async def apply_complex_mappings(self, data: Dict[str, Any], 
                                   mapping_config: Dict[str, Any], 
    ...
    async def _handle_name_combination(self, value: Any, config: Dict[str, Any],
                                     direction: str) -> Any:
    ...
    async def _handle_string_to_int_mapping(self, value: Any, config: Dict[str, Any], 
                                          direction: str) -> Any:
            ...
            if direction == "frappe_to_supabase":
                ...
                if isinstance(value, str) and value.startswith("PROJ-"):
                    ...
                    try:
                        ...
                        logger.warning(f"Could not convert project string {value} to integer")
                        return None
                return value
            else:  # supabase_to_frappe
    ...
    async def _handle_reverse_name_combination(self, value: Any, config: Dict[str, Any], 
                                            direction: str) -> Any:
    ...
    async def _handle_project_lookup(self, value: Any, config: Dict[str, Any], 
                                   direction: str) -> Any:
        ...
        try:
            ...
            if direction == "frappe_to_supabase":
                ...
                if records:
                    ...
                    logger.warning(f"Project lookup failed: {value} not found in {supabase_table}")
                ...
                if records:
                    ...
                    logger.warning(f"Project lookup failed: {value} not found in {frappe_doctype}")
                    return value
    ...
    async def _handle_date_fallback_mapping(self, data: Dict[str, Any], mapping: Dict[str, Any], 
                                          source_system: str, target_system: str) -> Any:
        ...
        try:
            ...
            if not primary_field or not fallback_field:
                logger.warning("Date fallback mapping missing primary_field or fallback_field")
                return None
            ...
            if fallback_field in data and data[fallback_field] is not None:
                logger.info(f"Using fallback field {fallback_field} for {primary_field}")
                return data[fallback_field]
            
            # Both fields are None or not present
            logger.info(f"Both {primary_field} and {fallback_field} are None or not present")
            return None
    ...
    async def _handle_default_value_mapping(self, data: Dict[str, Any], mapping: Dict[str, Any], 
                                          source_system: str, target_system: str) -> Any:
    ...
    async def _handle_date_format_conversion(self, value: Any, config: Dict[str, Any], 
                                           source_system: str, target_system: str) -> Any:
        """Handle date format conversion - convert ISO datetime to date string"""
        try:
            logger.debug(f"Date format conversion called with value: {value}, config: {config}")
            if not value:
                return value
            ...
            if input_format == "iso_datetime" and output_format == "date_string":
                ...
                logger.warning(f"Unsupported date format conversion: {input_format} -> {output_format}")
                return value
