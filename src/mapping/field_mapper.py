...
from ..utils.phone_normalizer import extract_phone_from_data, get_phone_lookup_fields, get_email_lookup_fields
...
class FieldMapper:
    ...
    async def map_fields(self, data: Dict[str, Any], source_system: str, target_system: str, 
        ...
        try:
            ...
            if source_system not in valid_directions or target_system not in valid_directions:
                raise ValueError(f"Invalid direction: {source_system} to {target_system}")
            ...
            if source_system == "frappe":
                frappe_specific_fields = ["_assign", "__islocal", "__unsaved", "__user_tags", "__comments", "_user_tags", "_comments"]
                original_keys = list(data.keys())
                data = {k: v for k, v in data.items() if k not in frappe_specific_fields}
                filtered_keys = list(data.keys())
                logger.info(f"Filtered Frappe fields: {original_keys} -> {filtered_keys}")
            
            # Apply complex mappings first (including email priority) to original data
            data = await self._apply_complex_mappings(data, complex_mappings, source_system, target_system)
            ...
            for source_field, target_field in field_mappings.items():
                if source_field in data:
                    ...
                    elif source_field == "status" and target_field == "_legacy_is_active" and source_field in data:
                        # The complex mapping should have transformed status to a boolean
                        # Check if the complex mapping was applied
                        if source_field in data and isinstance(data[source_field], bool):
                    ...
                    # For company field that was processed by complex mapping, use the transformed value
                    elif source_field == "company" and target_field == "organization_id" and source_field in data:
                        # The complex mapping should have transformed company to organization_id
                        # Check if the complex mapping was applied
                        if source_field in data and isinstance(data[source_field], str) and len(data[source_field]) == 36:
                            # This looks like a UUID
                            mapped_data[target_field] = data[source_field]
                        else:
                            # Fallback to original value if complex mapping wasn't applied
                            mapped_data[target_field] = data[source_field]
                    elif target_field == "id" and source_system == "frappe" and target_system == "supabase":
                        ...
                        if target_field in data and data[target_field] is None:
                            logger.warning(f"Skipping field {target_field} due to failed complex mapping")
                            continue
            ...
            mapped_data = await self._apply_email_priority_mapping(mapped_data, mapping, source_system, target_system)
            ...
            # Debug logging
            logger.info(f"Final mapped data for {source_system} -> {target_system}: {mapped_data}")
    ...
    async def _apply_email_priority_mapping(self, data: Dict[str, Any], mapping: Dict[str, str], 
        ...
        try:
            if source_system == "supabase" and target_system == "frappe":
                ...
                email_priority = ["personal_email", "company_email", "preferred_contact_email"]
    ...
    async def _apply_complex_mappings(self, data: Dict[str, Any], complex_mappings: Dict[str, Any], 
                                    source_system: str, target_system: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "email" in complex_mappings and complex_mappings["email"].get("type") == "email_priority":
                if source_system == "frappe" and target_system == "supabase":
                    email_priority = complex_mappings["email"].get("email_priority", ["personal_email", "company_email"])
                    for email_field in email_priority:
                        if email_field in data and data[email_field]:
                            data["email"] = data[email_field]
                            break
            ...
            if "name" in complex_mappings and complex_mappings["name"].get("type") == "name_combination":
                if source_system == "frappe" and target_system == "supabase":
                    ...
                    logger.debug(f"Split name '{full_name}' into first_name='{data['first_name']}' and last_name='{data['last_name']}'")
                    # Remove the combined name field
                    if "name" in data:
                        del data["name"]
            
            # Handle reverse name combination mapping specially
            logger.debug(f"Checking reverse name combination: first_name in complex_mappings: {'first_name' in complex_mappings}")
            if "first_name" in complex_mappings:
                logger.debug(f"first_name complex mapping: {complex_mappings['first_name']}")
            if "first_name" in complex_mappings and complex_mappings["first_name"].get("type") == "reverse_name_combination":
                logger.debug(f"Applying reverse name combination for {source_system} -> {target_system}")
                if source_system == "supabase" and target_system == "frappe":
                    # Split combined name back to first_name and last_name
                    full_name = data.get("name", "")
                    name_parts = full_name.split(" ", 1)
            ...
            for field_name, complex_config in complex_mappings.items():
                ...
                if field_name == "organization_id" and "organization_id" in data:
                    logger.debug(f"Applying organization_id complex mapping: {data['organization_id']} -> {complex_config}")
                    mapped_value = await self.complex_mapper.handle_complex_mapping(
                        data["organization_id"], complex_config, 
                        f"{source_system}_to_{target_system}", source_system, target_system
                    )
                    logger.debug(f"Complex mapping result: {mapped_value}")
                    data["company"] = mapped_value
                elif field_name == "start_date" and complex_config.get("type") == "date_format_conversion":
                    # Handle date format conversion for start_date
                    if field_name in data:
                        mapped_value = await self.complex_mapper.handle_complex_mapping(
                            data[field_name], complex_config, 
                            f"{source_system}_to_{target_system}", source_system, target_system
                        )
                        data[field_name] = mapped_value
                elif field_name == "end_date" and complex_config.get("type") == "date_format_conversion":
                    ...
                    if field_name in data:
                        ...
                        data[field_name] = mapped_value
                elif field_name == "act_start_date" and complex_config.get("type") == "date_fallback":
                    # Handle date fallback for act_start_date
                    mapped_value = await self.complex_mapper._handle_date_fallback_mapping(
                        data, complex_config, source_system, target_system
                    )
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "act_end_date" and complex_config.get("type") == "date_fallback":
                    ...
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "organization_id" and complex_config.get("type") == "default_value":
                    # Handle default value for organization_id
                    mapped_value = await self.complex_mapper._handle_default_value_mapping(
                        data, complex_config, source_system, target_system
                    )
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "gender" and complex_config.get("type") == "default_value":
                    ...
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "date_of_birth" and complex_config.get("type") == "default_value":
                    ...
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "date_of_joining" and complex_config.get("type") == "default_value":
                    ...
                    mapped_value = await self.complex_mapper._handle_date_format_conversion(
                        data.get(field_name), complex_config, source_system, target_system
                    )
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name == "end_date" and complex_config.get("type") == "date_format_conversion":
                    ...
                    if mapped_value is not None:
                        data[field_name] = mapped_value
                elif field_name in data and complex_config.get("type") != "email_priority":
                    mapped_value = await self.complex_mapper.handle_complex_mapping(
                        data[field_name], complex_config, 
    ...
    def get_primary_identifier(self, data: dict, source_system: str, target_system: str) -> Optional[tuple]:
        """
    ...
    def _is_task_record(self, data: dict, source_system: str, target_system: str) -> bool:
        """Check if this is a Task record based on available fields"""
        task_fields = ["subject", "task_name", "description", "page_content", "progress", "is_milestone"]
