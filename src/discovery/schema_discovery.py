...
class SchemaDiscovery:
    ...
    async def discover_all_schemas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            mappings = await self.create_intelligent_mappings(frappe_schemas, supabase_schemas)
    ...
    async def discover_frappe_schemas(self) -> Dict[str, Any]:
        ...
        try:
            doctypes = settings.frappe_discovery_doctypes.split(',') if hasattr(settings, 'frappe_discovery_doctypes') else []
            skip_fields = settings.frappe_discovery_skip_fields.split(',') if hasattr(settings, 'frappe_discovery_skip_fields') else []
            ...
            for doctype in doctypes:
                ...
                try:
                    ...
                    logger.warning(f"Failed to discover Frappe doctype {doctype}", error=str(e))
    ...
    async def _get_frappe_doctype_schema(self, doctype: str, skip_fields: List[str]) -> Optional[Dict[str, Any]]:
    ...
    async def discover_supabase_schemas(self) -> Dict[str, Any]:
        ...
        try:
            tables = settings.supabase_discovery_tables.split(',') if hasattr(settings, 'supabase_discovery_tables') else []
            skip_tables = settings.supabase_discovery_skip_tables.split(',') if hasattr(settings, 'supabase_discovery_skip_tables') else []
            skip_fields = settings.supabase_discovery_skip_fields.split(',') if hasattr(settings, 'supabase_discovery_skip_fields') else []
            ...
            for table in tables:
                ...
                try:
                    ...
                    logger.warning(f"Failed to discover Supabase table {table}", error=str(e))
    ...
    async def _get_supabase_table_schema(self, table: str, skip_fields: List[str]) -> Optional[Dict[str, Any]]:
        """Get detailed schema for a Supabase table"""
        try:
            # Get table schema using RPC function
            schema_info = await self.supabase_client.execute_rpc('get_table_schema', {'table_name': table})
    ...
    async def create_intelligent_mappings(self, frappe_schemas: Dict[str, Any], 
                                        supabase_schemas: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _find_best_table_match(self, doctype: str, supabase_schemas: Dict[str, Any]) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    async def _create_field_mapping(self, doctype: str, frappe_schema: Dict[str, Any], 
                                  table_name: str, supabase_schema: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for frappe_field in frappe_schema.get('fields', []):
                best_match = self._find_best_field_match(frappe_field, supabase_schema.get('fields', []))
                
                if best_match:
                    supabase_field = best_match
                    field_mappings[frappe_field['fieldname']] = supabase_field['fieldname']
            ...
            return {
                ...
                'confidence_score': self._calculate_mapping_confidence(field_mappings, sync_fields),
                'created_at': datetime.utcnow().isoformat()
            }
            
        except Exception as e:
            logger.error(f"Failed to create field mapping for {doctype} -> {table_name}", error=str(e))
            return None
    
    def _find_best_field_match(self, frappe_field: Dict[str, Any], supabase_fields: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_field_similarity(self, frappe_field: Dict[str, Any], supabase_field: Dict[str, Any]) -> float:
        ...
        type_compatibility = self._check_type_compatibility(
            frappe_field.get('fieldtype'), 
            supabase_field.get('fieldtype')
        )
    ...
    def _calculate_mapping_confidence(self, field_mappings: Dict[str, str], sync_fields: List[str]) -> float:
