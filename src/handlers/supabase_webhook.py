...
class SupabaseWebhookHandler:
    ...
    async def process_webhook(
        ...
        try:
            ...
            if not mapping:
                logger.info(
                    "Table not configured for sync, skipping", table=webhook_data.table
                )
            ...
            logger.error(
                "Failed to process Supabase webhook", error=str(e), payload=payload
            )
            raise HTTPException(
    ...
    def _map_supabase_operation(self, supabase_operation: str) -> str:
        ...
        operation_mapping = {"INSERT": "create", "UPDATE": "update", "DELETE": "delete"}
        return operation_mapping.get(supabase_operation, "update")

    async def handle_record_created(
    ...
    async def handle_record_updated(
        self,
        table: str,
        record: Dict[str, Any],
