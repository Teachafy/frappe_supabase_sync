...
class SyncQueue:
    ...
    async def enqueue_operation(self, operation: SyncOperation) -> bool:
        ...
        try:
            ...
            logger.info("Operation enqueued", operation_id=operation.id, queue=self.queue_name)
            return True
            
        except Exception as e:
            logger.error("Failed to enqueue operation", operation_id=operation.id, error=str(e))
            return False
    ...
    async def dequeue_operation(self) -> Optional[SyncOperation]:
        ...
        try:
            ...
            operation_data = self.redis_client.rpoplpush(self.queue_name, self.processing_queue_name)
        ...
        try:
            ...
            for operation_data in operations:
                ...
                if operation_dict["id"] == operation_id:
                    self.redis_client.lrem(self.processing_queue_name, 1, operation_data)
                    logger.info("Operation marked as completed", operation_id=operation_id)
                    return True
            
            logger.warning("Operation not found in processing queue", operation_id=operation_id)
            return False
            
        except Exception as e:
            logger.error("Failed to mark operation as completed", operation_id=operation_id, error=str(e))
            return False
    
    async def mark_operation_failed(self, operation_id: str, error_message: str) -> bool:
        ...
        try:
            ...
            for operation_data in operations:
                ...
                if operation_dict["id"] == operation_id:
                    ...
                    self.redis_client.lpush(self.failed_queue_name, json.dumps(operation_dict))
                    self.redis_client.lrem(self.processing_queue_name, 1, operation_data)
                    
                    logger.info("Operation marked as failed", operation_id=operation_id, error=error_message)
                    return True
            ...
            logger.error("Failed to mark operation as failed", operation_id=operation_id, error=str(e))
            return False
        ...
        try:
            ...
            for operation_data in failed_operations:
                ...
                if retry_count < max_retries:
                    ...
                    retried_count += 1
                    logger.info("Operation retried", operation_id=operation_dict["id"], retry_count=retry_count + 1)
    ...
    async def get_queue_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_operations": main_queue_size + processing_queue_size + failed_queue_size
            }
    ...
    async def clear_queue(self, queue_name: Optional[str] = None) -> bool:
        ...
        try:
            if queue_name:
                ...
                self.redis_client.delete(self.queue_name, self.processing_queue_name, self.failed_queue_name)
    ...
    async def get_operation_by_id(self, operation_id: str) -> Optional[SyncOperation]:
        ...
        try:
            # Check all queues
            for queue_name in [self.queue_name, self.processing_queue_name, self.failed_queue_name]:
    ...
    async def get_failed_operations(self, limit: int = 100) -> List[SyncOperation]:
        ...
        try:
            failed_operations = []
            operations_data = self.redis_client.lrange(self.failed_queue_name, 0, limit - 1)
