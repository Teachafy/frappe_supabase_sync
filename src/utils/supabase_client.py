...
class SupabaseClient:
    ...
    async def find_record_by_field(self, table: str, field: str, value: str) -> Optional[Dict[str, Any]]:
        """Find a record by a specific field value"""
        async def _find_record_by_field():
            response = self.client.table(table).select("*").eq(field, value).limit(1).execute()
            if response.data:
                return response.data[0]
            return None
        ...
        try:
            ...
            logger.error("Failed to find record by field", table=table, field=field, value=value, error=str(e))
            return None
    ...
    async def count_records(self, table: str, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            logger.error("Failed to count records", table=table, filters=filters, error=str(e))
            return 0
    ...
    async def _update_record(self, table: str, record_id: str, data: Dict[str, Any], **kwargs) -> Optional[Dict[str, Any]]:
    ...
    async def get_documents(self, table: str, filters: Optional[Dict[str, Any]] = None, 
        ...
        try:
            return await retry_with_exponential_backoff(
                _get_documents,
                retry_config=create_retry_config(max_retries=4)
            )
        except Exception as e:
